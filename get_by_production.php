...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (!isset($_GET['id'])) {
        echo json_encode(["status" => false, "message" => "production id is required."]);
        return;
    }

    // Search for movie in database
    $stmt = $connection->prepare('SELECT * FROM movies WHERE producer_id = ? LIMIT 20');
    $stmt->execute(array($_GET['id']));
    $movies = $stmt->fetchAll(PDO::FETCH_ASSOC);

    if (!$movies) {
        echo json_encode(["status" => false, "message" => "Invalid production id provided."]);
        return;
    }
